...
En el presente ejercicio vamos a trabajar con un diccionario (viajes_diarios) que registrará los usuarios que 
...
Implementar la función viajes_por_dia() que dado un diccionario viajes_diarios y una lista usuarios devuelva un nuevo diccionario,
...
def viajes_por_dia(viajes_diarios: dict[int, List[str]],  usuarios:List[str])-> dict[str, int]:
  ...
  for i in range(len(usuarios)):
    diasQueViajo: int = 0
    usuario: str = usuarios[i]
    for valores in viajes_diarios.values():
      if usuario in valores:
        diasQueViajo+=1
    res[usuario] = diasQueViajo
  return res