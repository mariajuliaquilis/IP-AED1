...
longitud mapeo | mapeo == [] = 0
               | otherwise = 1 + longitud(tail mapeo)

hayQueCodificar :: Char -> [(Char, Char)] -> Bool
hayQueCodificar c mapeo | longitud(mapeo) == 0 = False 
                        | otherwise = c == (fst(head mapeo)) || hayQueCodificar c (tail mapeo)
...
problema cuantasVecesHayQueCodificar (c: Char, frase: seq⟨Char⟩, mapeo: seq⟨Char x Char⟩ ) : Z {
  ...
  asegura: {(res = 0 y hayQueCodificar (c, mapeo) = false) o (res = cantidad de veces que c aparece en frase y 
            hayQueCodificar (c, mapeo) = true)}
}
-}
cantApariciones :: [Char] -> Char -> Integer
cantApariciones frase c | longitud(frase) == 0 = 0
                        | head(frase) == c = 1 + cantApariciones (tail frase) c 
                        | otherwise = cantApariciones (tail frase) c
...
-}
masCodificaciones :: [Char] -> [(Char, Char)] -> Integer -> Char -> Char
masCodificaciones frase mapeo cantidad caracter | longitud(frase) == 1 = caracter
                                                | cantCodificaciones > cantidad = masCodificaciones (tail frase) mapeo cantCodificaciones (head frase)
                                                | otherwise = masCodificaciones (tail frase) mapeo cantidad caracter
                                                where cantCodificaciones = cuantasVecesHayQueCodificar (head frase) frase mapeo

laQueMasHayQueCodificar :: [Char] -> [(Char, Char)] -> Char
laQueMasHayQueCodificar frase mapeo = masCodificaciones frase mapeo (cuantasVecesHayQueCodificar (head frase) frase mapeo) (head frase)
...
problema codificarFrase (frase: seq⟨Char⟩, mapeo: seq⟨Char x Char⟩ ) : seq ⟨Char⟩ {
  ...
  requiere: {|frase| > 0 }
  asegura: {|res| = | frase|}
  asegura: { Para todo 0 <= i < |frase| si hayQueCodificar(frase[i], mapeo) = true entonces res[i] = (mapeo[j])_1, para un j 
             tal que 0 <= j < |mapeo| y (mapeo[j])_0 = frase[i]}
...
{-Ejercicio 5 (1 punto)
Conteste marcando la opción correcta. Si un usuario no cumple con la precondición de la especificación de un programa y el programa 
no termina (se cuelga):
